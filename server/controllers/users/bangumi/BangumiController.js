...
let getNewBangumis = (req, res) => {
    //console.log("getBangumiAll")
    let sql = 'select type_id,bangumi_id as film_id,name,imgSrc,is_ended,weekday from pk_bangumi where is_ended=0 order by create_time desc';
    let sqlArr = [];
    let callback = async (err, data) => {
        if (err) {
            console.error("error", err.message);
...
module.exports = {
    getAll, getOne, getStyles, search, getRank,getNewBangumis
}